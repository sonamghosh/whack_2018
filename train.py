...
def train(net: DaRnnNet, train_data: TrainData, t_cfg: TrainConfig, n_epochs=10, save_plots=False):
    ...
    for e_i in range(n_epochs):
        ...
        if e_i % 10 == 0:
            ...
            # needs MSE for multiple input
            val_loss = y_test_pred - train_data.targs[t_cfg.train_size:]
            logger.info(f"Epoch {e_i:d}, train loss: {epoch_losses[e_i]:3.3f}, val_loss: {np.mean(np.abs(val_loss))}.")
...
def adjust_learning_rate(net: DaRnnNet, n_iter: int):
    # Should use Cosine Annealing with warm restarts https://www.jeremyjordan.me/nn-learning-rate/
...
if interactive_plot == True:
    ...
    py.iplot(data, filename='finalpredicted_05', auto_open=True)
